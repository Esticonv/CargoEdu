    ...
    : class, IEntity<TPrimaryKey>
{
    ...
    protected Repository(DbContext context, IMediator mediator) : base(context)
    {
        _mediator = mediator;
    }
    ...
    {
        BaseEvent[]? domainEvents = Repository<T, TPrimaryKey>.GetDomainEvents(Context);
        Context.SaveChanges();
        PublishDomainEvents(domainEvents).GetAwaiter().GetResult();
    }
    ...
    private static BaseEvent[]? GetDomainEvents(DbContext? context)
    {
        if (context == null)
        {
            return null;
        ...
        BaseEvent[] domainEvents = entities
    ...
    private async Task PublishDomainEvents(BaseEvent[]? domainEvents)
    {
